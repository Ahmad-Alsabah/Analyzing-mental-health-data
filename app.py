...
try:
    ...
    def draw_small_plot(fig):
        st.pyplot(fig, use_container_width=True)  # تعديل هنا

    # 👤 توزيع الجنس
    st.subheader("👤 توزيع الجنس")
    fig1, ax1 = plt.subplots(figsize=(4, 3))  # تكبير الحجم
    sns.countplot(x='Gender', data=df, ax=ax1, palette='Set2', width=0.4)
    ax1.set_title("Gender", fontsize=9)
    ax1.set_xlabel("")
    ax1.set_ylabel("")
    ...
    ax2.set_title("Stress?", fontsize=9)
    ax2.set_xlabel("")
    ax2.set_ylabel("")
    ...
    ax3.set_title("Support", fontsize=9)
    ax3.set_xlabel("")
    ax3.set_ylabel("")
    ...
    ax4.set_title("Countries", fontsize=9)
    ax4.set_xlabel("")
    ax4.set_ylabel("")
